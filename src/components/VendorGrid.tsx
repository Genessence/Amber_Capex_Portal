...
const FD_CELL_BASE = "sticky right-0 z-10 px-3 py-2.5 bg-slate-100 min-w-[200px]"
const FD_CELL_STYLE: React.CSSProperties = {
  borderLeft: "2px solid #171717",
  borderBottom: "1px solid #e2e8f0",
}
const FD_INPUT =
  "border border-slate-200 rounded-md px-2.5 py-1.5 text-xs w-full text-right focus:outline-none focus:ring-2 focus:ring-slate-400/50 bg-white placeholder:text-slate-300"
...
const TD_LAST: React.CSSProperties = { borderRight: "2px solid #171717", borderBottom: "1px solid #94a3b8" }
...
const ITEM_COL_ODD: React.CSSProperties  = { borderRight: "1px solid #94a3b8", borderBottom: "1px solid #94a3b8", background: "#FAFAFA" }

/* Vendor (Q) column header borders */
const Q_TH_FIRST: React.CSSProperties = { borderLeft: "2px solid #171717", borderBottom: "1px solid #e2e8f0" }
...
const Q_BODY_FIRST: React.CSSProperties = { borderLeft: "2px solid #171717", borderBottom: "1px solid #94a3b8" }
const Q_BODY_REST: React.CSSProperties  = { borderLeft: "1px solid #94a3b8", borderBottom: "1px solid #94a3b8" }
const Q_BODY_LOW: React.CSSProperties   = { borderLeft: "2px solid #059669", borderBottom: "1px solid #94a3b8" }

/* Attribute / currency row vendor cell borders */
const ATTR_Q_FIRST: React.CSSProperties = { borderLeft: "3px solid #171717", borderBottom: "1px solid #94a3b8" }
...
const TOTAL_Q_FIRST: React.CSSProperties = { borderLeft: "3px solid #171717" }
const TOTAL_Q_REST: React.CSSProperties  = { borderLeft: "1px solid #cbd5e1" }

/* Offer column borders */
const OFFER_BORDER: React.CSSProperties   = { borderLeft: "2px solid #64748B", borderBottom: "1px solid #94a3b8" }
const OFFER_TOTAL: React.CSSProperties    = { borderLeft: "2px solid #64748B" }
const ADD_OFFER_STYLE: React.CSSProperties = { borderLeft: "2px dashed #64748B", borderBottom: "1px solid #e2e8f0" }
const ADD_OFFER_TOTAL: React.CSSProperties = { borderLeft: "2px dashed #64748B" }

/* Final Decision borders */
const FD_BODY_STYLE: React.CSSProperties  = { borderLeft: "2px solid #171717", borderBottom: "1px solid #e2e8f0" }
const FD_TOTAL_STYLE: React.CSSProperties = { borderLeft: "2px solid #171717" }

/* Attribute label cell */
const ATTR_LABEL_STYLE: React.CSSProperties = { borderRight: "3px solid #171717", borderBottom: "1px solid #e2e8f0", minWidth: "0" }
const SEND_LABEL_STYLE: React.CSSProperties  = { borderRight: "3px solid #171717", borderBottom: "1px solid #e2e8f0" }
const TOTAL_LABEL_STYLE: React.CSSProperties = { borderRight: "3px solid #171717" }
const TOTAL_ROW_STYLE: React.CSSProperties   = { borderTop: "2px solid #171717" }
...
function quoteCellStyle(
  ...
  return isLowest
    ? { borderLeft: "3px solid #059669", borderBottom: "1px solid #e2e8f0" }
    : isApproved
    ? { borderLeft: "3px solid #64748B", borderBottom: "1px solid #e2e8f0" }
    : { borderLeft: "1px solid #e2e8f0", borderBottom: "1px solid #e2e8f0" }
}

function LowestChip({ label = "Lowest" }: { label?: string }) {
  return (
    <span className="inline-flex items-center gap-0.5 ml-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 leading-none align-middle">
      ↓ {label}
    </span>
  )
}
...
const ItemBodyRow = React.memo(function ItemBodyRow({
  ...
  return (
    <tr className={rowBg}>
        ...
        return (
          <td key={inv.id}
            className={["px-2 py-2 text-center text-xs",
              isLowestItem ? "bg-emerald-50" : isApproved ? "bg-slate-50/60" : ""].join(" ")}
      ...
      {(isSourcing || isLocked) && (
        <td className="sticky right-0 z-10 px-2 py-2 bg-[#F4F4F5] min-w-[210px]" style={FD_BODY_STYLE}>
          {isLocked ? (
            <div className="space-y-0.5">
              ...
              {Number(finalDisc) > 0 && (
                <p className="text-[10px] text-slate-600">{finalDisc}% discount</p>
              )}
              <p className="text-[11px] text-slate-600 truncate">
                {vendors.find(v => v.id === finalVendorId)?.vendorName ?? <span className="text-slate-300">—</span>}
              </p>
              <p className="text-[11px] font-bold text-slate-700 text-right border-t border-slate-300 pt-0.5 mt-0.5">
            ...
            <>
              ...
              <div className="mb-1">
                <p className="text-[10px] font-bold text-slate-700 uppercase mb-0.5">Select Vendor</p>
                <select
                  value={finalVendorId}
                  onChange={e => onSetFinalVendor(item.id, e.target.value)}
                  className="border border-slate-200 rounded px-1.5 py-1 text-xs w-full bg-white focus:outline-none focus:ring-1 focus:ring-slate-400 text-slate-700"
                >
...
function ItemRowGrid({ request, invites, vendors, currentRole, onSelectFinal, auctionSelectedVendorIds = [], onAuctionSelectionChange }: Props) {
  ...
  const TH_FIXED = "bg-[#171717] text-white px-2 py-2.5 text-[11px] font-bold uppercase tracking-wider"
    ...
    <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
        ...
        <table className="w-full border-collapse" aria-label="Request comparison grid">
          ...
          <thead>
            <tr style={TOTAL_ROW_STYLE}>
              ...
              <th scope="col" className={`${TH_FIXED} w-14 text-right`} style={{ borderRight: "2px solid #171717" }}># Qty</th>

              {vendorCols.map(({ inv, vendor, latestQuote }, colIdx) => {
                const isApproved = inv.status === "approved"
                return (
                  <th key={inv.id} scope="col"
                    className={[TH_Q, isApproved ? "!bg-slate-700" : ""].join(" ")}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center gap-1 mt-1">
                      ...
                      {/* Approval/award is done only in the Final Decision area (per-line vendor +
                          price → FinalDecisionActions), never per-vendor here. */}
                    </div>
                  </th>
                )
              })}
              ...
              {visibleOfferCols.map(col => {
                ...
                const existingInvite = col.vendorId
                  ...
                  : undefined
                const headerBg = col.sent ? "bg-[#171717]" : "bg-slate-500"
                return (
                      ...
                      <>
                        <div className="flex items-center gap-1 mb-1">
                          <select value={col.vendorId} onChange={e => setOfferVendor(col.id, e.target.value)}
                            className="flex-1 text-[11px] border border-slate-300 rounded px-1.5 py-1 bg-white text-slate-700 focus:outline-none focus:ring-1 focus:ring-slate-400 w-full">
                            <option value="">Select vendor…</option>
                        ...
                        <div className="flex items-center justify-center gap-1.5">
                          <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-800">+ Offer</span>
                          {existingInvite && (
                            <button onClick={() => copyLink(existingInvite)} title="Copy supplier link"
                              className="p-0.5 rounded bg-white/20 hover:bg-white/40 text-white">
                              <Mail className="w-3 h-3" />
              ...
              {isSourcing && !isLocked && (
                <th scope="col" onClick={addOfferCol}
                  className="px-2 py-2 text-center w-16 bg-slate-50 cursor-pointer hover:bg-slate-50 transition-colors"
              ...
              {(isSourcing || isLocked) && (
                <th scope="col" className={TH_FD} style={FD_BODY_STYLE}>
                  ...
                  {isLocked
                    ? <p className="text-[10px] text-slate-300 mt-0.5 font-semibold">🔒 Locked</p>
          ...
          <tbody>
                    ...
                    <td key={col.id}
                      ...
                      {col.sent ? (
                        ...
                        <input type="number" placeholder="0" value={val}
                          ...
                          className="w-full text-xs text-right border border-slate-200 rounded px-1.5 py-0.5 bg-white focus:outline-none focus:ring-1 focus:ring-slate-400 placeholder:text-slate-300"
                        />
                      )}
                    </td>
                  )
            ...
            <tr className="bg-slate-50/70">
              ...
              {visibleOfferCols.map(col => {
                ...
                return (
                  <td key={col.id}
                    ...
                    style={OFFER_BORDER}>
                    {col.sent ? (
                      <span className="text-xs font-semibold text-slate-800">{cur}</span>
                    ) : (
                      <select value={cur} onChange={e => setOfferAttr(col.id, "currency", e.target.value)}
                        className="border border-slate-200 rounded px-1.5 py-0.5 text-xs w-full bg-white focus:outline-none focus:ring-1 focus:ring-slate-400">
              ...
              {(isSourcing || isLocked) && (
                <td className="sticky right-0 z-10 px-3 py-2 bg-slate-100 min-w-[190px]" style={FD_BODY_STYLE}>
                  {isLocked ? (
                    <p className="text-xs font-semibold text-slate-900 text-right">{fdAttr.currency}</p>
                  ) : (
                    <select value={fdAttr.currency} onChange={e => setFdAttrField("currency", e.target.value)}
                      className="border border-slate-200 rounded px-2 py-1 text-xs w-full bg-white focus:outline-none focus:ring-2 focus:ring-slate-400/50">
            ...
            <tr style={TOTAL_ROW_STYLE} className="bg-slate-100">
              ...
              {(isSourcing || isLocked) && (
                <td className="sticky right-0 z-10 px-3 py-2 bg-slate-200 min-w-[210px]" style={FD_TOTAL_STYLE}>
            ...
            {isSourcing && !isLocked && visibleOfferCols.length > 0 && (
              <tr className="bg-slate-50/30">
                <td colSpan={3}
                  className="sticky left-0 z-10 px-3 py-2 text-xs font-semibold text-slate-400 bg-slate-50/50 whitespace-nowrap"
                ...
                {visibleOfferCols.map(col => (
                  <td key={col.id}
                    className={["px-2 py-2 text-center", col.sent ? "bg-slate-50/50" : "bg-slate-50/60"].join(" ")}
                    style={OFFER_BORDER}>
                    {col.sent ? (
                      <span className="text-[11px] font-semibold text-slate-700">✓ Sent</span>
                    ) : (
                      <button
                        onClick={() => handleSendQuoteToSupplier(col)}
                        disabled={!col.vendorId}
                        className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-slate-500 hover:bg-slate-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-[11px] font-semibold transition-colors whitespace-nowrap"
                      >
                        <Mail className="w-3 h-3" />
                        Send Quote
                      </button>
                ...
                <td className="sticky right-0 z-10 bg-slate-50 min-w-[190px]" style={FD_BODY_STYLE} />
              </tr>
            )}
          </tbody>
        </table>
      ...
      {isSourcing && !isLocked && ranAuction && (
        <div className="border-t border-slate-200 px-5 py-4 flex items-center justify-between gap-4 bg-white">
          ...
          <button
            onClick={handleSave}
            className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-white hover:bg-slate-50 text-slate-700 text-xs font-semibold border border-slate-200 transition-colors shrink-0"
          >
            Save Final Decision
          </button>
        </div>
        ...
        <div className="border-t border-slate-200 px-5 py-4 flex items-center justify-between gap-4 bg-white">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#000000] hover:bg-[#000000] text-white text-xs font-semibold transition-colors"
            >
              ✓ Approve
            </button>
          </div>
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-2 flex-wrap">
        <input value={tableSearch} onChange={e => setTableSearch(e.target.value)}
          ...
          className="flex-1 min-w-[160px] max-w-sm rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#171717]/50" />
        {isSourcing && (
                  ...
                  return (
                    <th key={quote.id} scope="col"
                      className={["px-3 py-3 text-center min-w-[168px]",
                        isLowest ? "bg-slate-50" : isApproved ? "bg-slate-50" : "bg-slate-50"].join(" ")}
                      ...
                      <span className={["mt-1.5 inline-flex items-center gap-1.5 text-xs font-semibold px-2 py-0.5 rounded-full", INVITE_STATUS_COLORS[invite.status] ?? "bg-slate-200 text-slate-600"].join(" ")}>
                ...
                {pendingInvites.map(inv => {
                  ...
                  return (
                    <th key={inv.id} scope="col"
                      ...
                      <div className="flex items-center justify-center gap-1 mt-2">
                        <button onClick={() => copyLink(inv)} className="p-1.5 rounded-md text-slate-300 hover:text-[#171717] hover:bg-[#E5E5E5] transition-colors"><Copy className="w-3.5 h-3.5" /></button>
                        <button onClick={() => setEmailOpenId(id => id === inv.id ? null : inv.id)} className={["p-1.5 rounded-md transition-colors", emailOpenId === inv.id ? "bg-[#E5E5E5] text-[#171717]" : "text-slate-300 hover:text-[#171717] hover:bg-[#E5E5E5]"].join(" ")}><Mail className="w-3.5 h-3.5" /></button>
                      </div>
                    </th>
                  )
                })}
                {isSourcing && (
                  <th scope="col" className="sticky right-0 z-20 px-4 py-3 min-w-[200px] bg-slate-100 text-left"
                    style={{ borderLeft: "2px solid #171717", borderBottom: "1px solid #e2e8f0" }}>
                    <p className="text-sm font-bold text-slate-900 mb-2">Final Decision</p>
                    <select value={finalInviteId} onChange={e => handleFinalVendorChange(e.target.value)}
                      className="border border-slate-200 rounded-md px-2.5 py-1.5 text-xs w-full bg-white focus:outline-none focus:ring-2 focus:ring-slate-400/50 text-slate-700">
                      <option value="">Select vendor…</option>
            ...
            <tbody>
              ...
              {invites.length > 0 && (
                <>
                  ...
                  <tr>
                    <td className="sticky left-0 z-10 bg-[#E5E5E5] px-4 py-3 text-xs font-bold text-[#000000] uppercase tracking-wider whitespace-nowrap w-44 min-w-[176px]"
                    ...
                    <tr>
                      ...
                      <td colSpan={allQuotes.length + pendingInvites.length} className="px-4 py-2.5 bg-white" style={{ borderTop: "1px solid #e2e8f0", borderBottom: "1px solid #e2e8f0" }}>
                        <input type="text" value={finalForm.remarks} onChange={e => setFinalForm(f => ({ ...f, remarks: e.target.value }))}
                          placeholder="Optional note to supplier…" className="border border-slate-200 rounded-md px-3 py-1.5 text-sm w-full max-w-md focus:outline-none focus:ring-2 focus:ring-slate-400/50" />
                      </td>
                      <td className="sticky right-0 z-10 px-4 py-2.5 bg-slate-100 min-w-[200px]"
                        style={{ borderLeft: "2px solid #171717", borderTop: "1px solid #e2e8f0", borderBottom: "1px solid #e2e8f0" }}>
                        <Button size="sm" onClick={sendFinalDecision} disabled={!finalForm.price}
                          className="w-full text-xs bg-[#171717] hover:bg-black text-white disabled:opacity-50">
                          Send Decision
                        </Button>
                      </td>
                    </tr>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                  <div className="bg-slate-50 border-b border-slate-200 px-4 py-3 space-y-1.5">
                    <div className="flex items-center gap-1.5 mb-1"><span className="text-xs font-bold px-2 py-0.5 rounded-full bg-slate-200 text-slate-700 uppercase">Sent</span><span className="text-xs text-slate-400">{new Date(inv.invitedAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span></div>
                  ...
                  <div className="px-4 py-3 text-xs text-slate-600 leading-relaxed space-y-3">
                    ...
                    <a href={supplierLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg bg-[#171717] text-white text-xs font-semibold hover:bg-[#000000] transition-colors"><Mail className="w-3.5 h-3.5" />Open Supplier Form</a>
                  </div>
                </div>
                <div className={["bg-white rounded-xl border overflow-hidden", latestQ ? "border-slate-200" : "border-slate-200 opacity-60"].join(" ")}>
                  <div className={["border-b px-4 py-3", latestQ ? "bg-slate-50 border-slate-200" : "bg-slate-50 border-slate-200"].join(" ")}>
                    <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                      {latestQ ? (
                        <>
                          <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-700 uppercase">Received · Q{inv.quotes.length}</span>
                          {latestQ.seededByBuyer && (
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-100 text-slate-900 border border-slate-300">Added at request</span>
                          )}
                        </>
                      ) : (
      ...
      {finalDecisionRows.length > 0 && (
        <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
          <div className="flex items-center justify-between px-5 py-3.5 bg-slate-50 border-b border-slate-200">
            ...
            {approvedInviteId && <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-slate-100 text-slate-700">Vendor Selected</span>}
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
