...
export default function PoIssuePage() {
  ...
  const target = useMemo(() => resolvePoTarget(token, requests, invites), [token, requests, invites])
  ...
  const pi = invite
    ? invite.proformaInvoice
    : invites.find(i => i.requestId === request?.id && i.vendorId === vendor?.id)?.proformaInvoice
    ...
    return (
      <Shell>
        <Terminal
          ...
          title="Loading…"
          note="Fetching the PO handoff details."
        />
      </Shell>
    )
  }

  // A Plant-Accounts token resolves to the same request but belongs to their page — reject it here.
  if (!target || !request || target.stage !== 'po_issue') {
    return (
      <Shell>
        <Terminal
  ...
  if (done || existingPo?.issuedAt || status === 'payment_in_progress' || status === 'completed') {
    ...
    const withTrial = done ? trialOnRequest : trialRequired
    return (
      <Shell>
        <div className={SUPPLIER_CARD}>
    ...
    return (
      <Shell>
        <Terminal
          ...
          note={`This request is currently "${STATUS_LABELS[status as keyof typeof STATUS_LABELS] ?? status}". Plant Accounts must submit the FA codes before you can issue the PO.`}
        />
      </Shell>
    )
  }
