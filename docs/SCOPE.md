...
| **Plant Accounts** (`plant_accounts`) | Global | Assign FA codes per line, then submit to Global Accounts |
| **Global Accounts** (`accounts`) | Global | Assign PO number, upload & issue the PO to the vendor, record milestone payments |
...
- **Budget proposals → admin approval → new FY** — the live Brown Field master FY is read-only; maintenance authors a next-FY proposal (seeded from the live FY, with **bulk Excel/CSV upload**) on **Budget Planning** (`/capex/budget-proposals`); super admin reviews a per-head diff and **publishes** it as the new live FY on **Budget Approvals** (`/capex/budget-approvals`).
...
- **Accounts & payments (split: Plant → Global)** — finalized vendor uploads a PI; **Plant Accounts** assign FA codes + submit — which first shows a **FA-code email preview** (`FaEmailModal`, simulated send to a dedicated, editable recipient listing the ordered items + FA codes; one email per submission) — then **Global Accounts** assign the PO number, **upload the PO document** and issue it (the vendor is notified and downloads it from the supplier portal), then tick payment milestones (advance/dispatch/installation) from the vendor's split. **Accounts Queue** at `/accounts/queue`.
...
- **INCO Terms (Incoterms 2020)** — sourcing can quick-add any vendor by name/email/phone; new/one-time vendors must approve a 12-question Incoterms agreement (two-sided edit/resend/accept/reject) **before they can quote**.
