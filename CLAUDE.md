...
| `(public)` | `/supplier/[token]`, `/approve/[token]`, `/po/[token]` | Tokenised public pages — wraps `CapexProvider` only, no auth. Supplier portal; plant-head approve/reject; **Global Accounts (Sandeep) PO issue** |
...
Roles: `buyer`, `buyer_jhajjar_p1`, `buyer_jhajjar_p2`, `sourcing_member`, `maintenance`, `plant_accounts`, `accounts`, `super_admin`. **The `plant_head*` and `sourcing_head` roles were removed (2026-07)** — plant-head approvals happen via a public emailed link (see **Workflow overhaul (2026-07)**), and the sourcing team finalizes vendors with no head gate.
...
Plant-scoped roles (`buyer_jhajjar_*`) filter data to their plant. `ROLE_PLANT` in `constants.ts` maps these roles to their plant value; roles absent from the map have access to all plants.

### Workflow overhaul (2026-07)

A set of changes reshaping approvals, sourcing, and fulfillment. **These supersede the older descriptions further down where they conflict.**

**Plant head via public email link (roles removed).** `plant_head*` and `sourcing_head` are removed from all registries/gates. Plant-head approval (budget + request) now happens through a **tokenised public page** `src/app/(public)/approve/[token]/page.tsx`, resolved by `resolveApprovalTarget(token, requests, budgetProposals)` in `tokenUtils.ts` (token minted by `generateApprovalToken` — CSPRNG; `buildApprovalLink`). The internal side shows a **copy-link + email-preview** affordance (`EmailPreviewModal`, the generalized `FaEmailModal`) at each handoff. On the request detail (`capex/[id]`) the plant-head approval panel (copy link + preview email) renders for **all internal roles including the buyer/requester** while `pending_head_approval`, so the requester can send the link to the plant head. `CapexRequest.approvalToken`/`BudgetProposal.approvalToken` carry the tokens (backfilled in `normalizeRequest`). Public mutations: `decideRequestPlantHead(requestId, decision)` and `decideBudgetPlantHead(id, decision)` (state-guarded). The context `storage` listener re-syncs `budgetProposals` too so a decision in the public tab reflects internally.

**Request routing.** `initialStatusForRequest`: **only Green Field skips head approval** (`→ sourcing`); Brown Field + Digitisation + IT all `→ pending_head_approval`.

**Budget approval = multi-stage.** `BudgetProposalStatus`: `draft → pending_plant_head → pending_admin → needs_correction → pending_accounts → approved | rejected`. `submitBudgetProposal` → `pending_plant_head` (rotates token, restarts from plant head on rework, clears prior-stage stamps); `decideBudgetPlantHead(id, decision, note?, editedItems?)` → `pending_admin` (approve) / `needs_correction` / `rejected`; `decideBudgetProposal(id, decision, actor, note?, editedItems?)` (super-admin) → `pending_accounts` (approve, **no publish**) / `needs_correction` / `rejected`; `decideBudgetAccounts` (global accounts) → `approved` + **publish to `capexMaster`** (reads target from committed state before mutating; guarded by `pending_accounts`). **Both the plant head (public page) and super admin can EDIT the line items + write a remark when sending back for correction** via the shared **`BudgetCorrectionPanel`** (`decide…(…, 'needs_correction', note, editedItems)` applies the edits atomically); the author sees the edited items + `correctionNote` and resubmits. `/capex/budget-approvals` serves `super_admin` (admin stage) **and** `accounts` (accounts stage). Authors: `maintenance`, `sourcing_member`, `super_admin`. **On `/capex/requests` the `maintenance` (budget-upload) user sees a BUDGET-approval-requests list** (their proposals + status + correction remark + Edit/Resubmit link), not item requests.

**Sourcing head removed → award goes straight to PI.** `canFinalize` = the sourcing team (`sourcing_member`/`super_admin`) everywhere (`FinalDecisionActions`, `VendorGrid`, `RfqPanel`, `capex/[id]`).

**Reverse auction: 5% + rank reset.** On escalation, `seedAuctionFromRfq` seeds each vendor's opening bid onto `VendorInvite.openingQuote` (**not** `quotes[]`, and `price` = base subtotal per the auction-bid convention) so `computeVendorRankings` stays empty until a fresh bid. `setAuctionConfig` sets `auctionConfig.openingBestPrice = min(current best, INR basis) × 0.95` — the displayed "price to beat". Award-price fallback (`resolveFinalVendor`, `VendorGrid` auto-fill) reads `quotes.at(-1) ?? openingQuote`.

**Trials (optional QA gate).** Sourcing toggles **Require an item trial** in a **"Sourcing setup"** card in the request-detail sourcing view (`capex/[id]`, visible pre-award to the sourcing team in BOTH RFQ and auction modes; `FinalDecisionActions` just shows an "ON" reminder). That same card has an **Import / Add Vendor** button (opens `VendorOnboardModal`) so vendors can be added in the auction path too, not just the RFQ panel; `awardAndRequestPi`/`finalizeSplitAward` propagate `request.trialRequired` → awarded invites. Types: `TrialStatus` (`not_required|pending_upload|pending_review|approved|rejected`), `TrialSubmission`, `TrialMessage` on both `CapexRequest` (single-vendor) and `VendorInvite` (awarded). After the **advance** milestone (first non-final) is paid, the vendor uploads via `submitTrial(inviteId)` (routes by `invite.awarded`); sourcing reviews with `respondToTrial(requestId, resp, inviteId?)` (approve → `approved`; reject → `rejected`, vendor re-uploads). `finalPaymentBlockedByTrial` blocks the `isFinal` milestone in `AccountsPanel` + `markPaymentMade` until approved. UI: `TrialCard` (`mode='upload'|'review'`), `trialUtils.ts` labels/colours. Both mutations are state-precondition-guarded.

**Delivery lead time → final-payment date.** Supplier delivery is now **days** (`RfqQuote.deliveryDays`/`Quote.deliveryDays`). `markPaymentMade` stamps `advancePaidAt` when the first non-final milestone is ticked; `expectedFinalPaymentDate(advancePaidAt, deliveryLeadDays(invite))` (`paymentUtils.ts`) is shown next to the final milestone.

**Fulfillment additions.** Multi-file PO: `PurchaseOrder.poDocuments[]` (offloaded to IndexedDB keyed `podoc:<id>`; capped at `MAX_PO_DOCS`). FA-code handoff email targets **`GLOBAL_ACCOUNTS_EMAIL`** ("Sandeep") with a **public PO-issue link** (`/po/[token]`, minted as `CapexRequest.poToken` / `VendorInvite.poToken` when the vendor submits the PI — `generatePoToken` / `buildPoLink` / `resolvePoTarget` in `tokenUtils.ts`; page at `src/app/(public)/po/[token]/page.tsx`). Sandeep issues the PO with **no portal login**; then the vendor **re-uploads the PI** (`piReuploadAllowed` + `resubmitProformaInvoice`), Accounts ticks **payment milestones**, and if `trialRequired` the vendor uploads a **trial after the advance** (final payment blocked until sourcing approves). Internal Global Accounts can still issue the PO from `AccountsPanel`. `plant_accounts` is in `PAYMENT_ROLES`. Post-PO PI re-upload: `piReuploadAllowed` (auto-set on PO issue) + `resubmitProformaInvoice`. The RfqPanel quick-add ("Invite new vendor") has a **Foreign vendor** checkbox → Incoterms.

**Foreign vendors + currency.** `Vendor.foreign` (onboard modal + `inviteNewVendor`) drives the Incoterms gate (`incoTermsBlocksQuote` keys on `foreign`, not `oneTime`); INCO seeded in `inviteVendors` for foreign vendors. **Currency:** new `src/lib/currencyUtils.ts` (`FX_TO_INR` static table, `toInr`, `formatCurrency`, `currencySymbol`); `fmtCurrency(n, currency)` in `auctionTheme.ts` delegates. All cross-vendor comparisons + award/threshold/PO amounts use an **INR basis** (`inrRfqTotal` in `rfqUtils.ts`, `toInr` in `paymentUtils`/`VendorGrid`/`capexContext`); sourcing surfaces show the INR value with the original foreign amount below.
...
| `src/lib/rfqUtils.ts` | RFQ price-flow status labels/helpers (`effectiveRfqStatus` w/ legacy tolerance, two-gate `canRequestPi`, `rfqTotal`, **`inrRfqTotal`** (INR basis), `rfqLineUnitPrice`, `rfqLineSubtotal`, `lowestRfqTotal`) |
| `src/lib/currencyUtils.ts` | Static `FX_TO_INR` table + `toInr`/`formatCurrency`/`currencySymbol` — the single source for currency conversion + symbols (no UI for the rates) |
| `src/lib/trialUtils.ts` | Trial (QA gate) status labels/colours + `effectiveTrialStatus` |
...
| `src/components/supplier/SupplierQuoteTable.tsx` | Vendor portal **desktop** pricing table (near-black sourcing-grid style; variants `read`/`entry`/`bid`) — line items as rows |
...
| `src/components/RfqPanel.tsx` | Sourcing RFQ panel: per-vendor price negotiation, per-vendor doc-checklist invite, B&W comparison grid |
| `src/components/FinalDecisionActions.tsx` | Shared Final-Decision approve + Request-PI bar (RFQ + auction): split award, bulk or per-vendor; carries the **Require item trial** toggle |
| `src/components/EmailPreviewModal.tsx` | Generalized email preview (recipient/subject/body + copyable approval **link**); `FaEmailModal` is now a thin wrapper |
| `src/components/TrialCard.tsx` | Trial upload (supplier) + review (sourcing) card |
| `src/components/BudgetCorrectionPanel.tsx` | Shared editable line-item table + remark + Send-Back, used by the super admin (approvals page) and plant head (public page) |
| `src/app/(public)/approve/[token]/page.tsx` | Public plant-head approval page (request + budget), resolved via `resolveApprovalTarget` |
| `src/app/(public)/po/[token]/page.tsx` | Public Global Accounts (Sandeep) PO-issue page, resolved via `resolvePoTarget` |
| `src/components/DocPackageReview.tsx` | Supplier-facing doc review (Commercial Terms/PBG/DLC/Payment Terms + custom `extraDocs`) + accept/decline |
...
| `src/components/Sidebar.tsx` | White collapsible sidebar, role-filtered nav, user footer |
...
- **Theme:** **black-and-white chrome** — white surfaces (`--background`/`--card`), near-black foreground (`--foreground`, `#171717`), **blue `#2563EB` as the single chrome accent** (`--primary`; CTAs, focus, interactive emphasis), and **red reserved for danger** (destructive/over-budget). Page chrome, tables, buttons, cards, sidebar and nav are grayscale + blue (+ red for danger); `slate-*` is the neutral. Subtle charcoal→black gradients in appropriate places (login hero, table headers). **Two colour exceptions on top of the B&W chrome:** (1) **status badges carry colour** (see below) for workflow visibility; (2) **value semantics** — the **lowest quote / "best price"** highlight and any **savings / under-budget** indicator render **green** (`emerald-*` — comparison-grid lowest cell + `↓ Lowest`/`L1`/`Best` chips, dashboard Money-Saved + Negotiated-Savings + Remaining, Saved-vs-Budget card, "under allocation" chip, supplier Best-Price), while any **loss / over-budget** indicator renders **red** (over-budget chip, "over allocation", Over-Budget card). Prefer semantic tokens (`bg-background`, `text-foreground`, `bg-primary`, `border-border`) over hardcoded hex in new UI work.
- **Coloured status badges (icon + semantic colour):** status/priority/invite badges are the one place colour is used freely — each is a light-tint chip with a coloured border and a **Lucide icon**, for clear at-a-glance state. `STATUS_COLORS` in `constants.ts` gives a **distinct colour per request status** (blue=submitted, amber=pending/awaiting, sky=sourcing, violet=negotiation, emerald=approved, orange=PI-requested, cyan=accounts, indigo=payment, green=completed, red=rejected). `BADGE_TONE` defines the shared 5-phase palette (`soft` slate → `pending` amber → `active` blue → `done` emerald → `danger` red) used by `INVITE_STATUS_COLORS` and the secondary maps (`RFQ_STATUS_COLORS`, `INCO_TERMS_STATUS_COLORS`, `DOC_APPROVAL_STATUS_COLORS`, `ADHOC_STATUS_COLORS`, `BUDGET_PROPOSAL_STATUS_COLORS`, `AUCTION_APPROVAL_STATUS_COLORS`). Request status renders via the shared **`StatusBadge`** component (`src/components/StatusBadge.tsx`) pairing colour + `STATUS_ICONS`; invite badges use `INVITE_STATUS_ICONS` inline. Use `StatusBadge` for any new request-status display rather than re-deriving spans.
...
- **Density tokens (`src/lib/uiTokens.ts`):** canonical compact class strings — `PAGE_SHELL`/`PAGE_STACK` (`p-5`), `SECTION_GAP` (`space-y-4`), `SECTION_GRID` (`gap-4`), `CARD` (`bg-card rounded-xl border border-border shadow-sm p-4`), `META_GRID` (`gap-x-6 gap-y-3`), `TABLE_HEAD` (near-black charcoal→black gradient, `from-neutral-800 to-neutral-900`), `TH_CELL`/`TD_CELL` (`py-2` — read-only data rows), `TD_CELL_INPUT` (`py-3`, the documented exception for editable input-grid cells), and `SUPPLIER_CARD` (the public vendor hero card). These are the canonical reference for the portal's rhythm; data tables apply the same `py-2` directly. Brand literals (the `NAVY`/`TEAL`/`TABLE_SURFACE` exports — names kept for back-compat — are now near-black `#171717`, blue `#2563EB`, and table surface `#F4F4F5`).
...
**Unified Final-Decision award + per-vendor documents (2026-06):** the RFQ and reverse-auction comparison grids are **pure black-and-white** (no blue; the request number renders black) and behave identically. **Approval + Request-PI live only in the Final Decision area** via the shared **`FinalDecisionActions`** component (`src/components/FinalDecisionActions.tsx`), rendered below the grid in **both** modes — there are **no per-vendor "✓ OK / Select as Final / Request PI" buttons** anymore. Sourcing fills the per-line Final Decision (selecting a vendor **auto-fills that line's price** from the vendor's quoted unit price — `Quote.itemPrices[itemId]` for auction, `rfqLineUnitPrice` for RFQ), then approves **all selected vendors at once OR each vendor separately**. Both call the new atomic context fn **`awardAndRequestPi(requestId, decision, actor, onlyVendorId?)`** (marks the vendor(s) `awarded` AND sets `awardStatus: 'pi_requested'` in one state pass, then bumps the request to `pi_requested`); omit `onlyVendorId` to award all, pass it to award one. **RFQ now does split award too** (different lines → different vendors, each its own award track), exactly like the auction — so RFQ is no longer single-vendor. `finalizeSplitAward` gained an `onlyVendorId` arg for additive per-vendor awarding. The auction footer keeps only a **Save Final Decision** button (persists `sourcingDecision` so the bar sees it); the auction must have **ended** before the bar's actions enable.

**Per-vendor document selection at invite (`DocSelection`):** when sourcing invites vendors (RFQ picker in `RfqPanel`), each selected vendor gets a **checklist of documents to send for approval** — Commercial Terms, PBG, DLC, Payment Terms (`DOC_OPTIONS` in `docPackageUtils.ts`) **plus "Add custom document"** (free-form name + text → `DocApprovalPackage.extraDocs`). The choice is per-vendor (`DocSelection` in `types.ts`: `{ commercialTerms?, pbg?, dlc?, paymentTerms?, extraDocs? }`), threaded through `inviteVendors(requestId, vendorIds, docSelections?)` / `inviteNewVendor(..., selection?)` into `buildDocApprovalPackage(vendor, { selection })`, which now **omits** any unselected document. `defaultDocSelection(vendor)` preserves the legacy set (Commercial Terms + PBG + DLC, + Payment Terms for one-time vendors) when no selection is passed. `DocPackageReview` renders `extraDocs` and only shows the documents actually sent; `docPackageTitles(pkg)` drives the (now dynamic) supplier decline copy.

**GST via HSN — item-wise (`src/lib/hsnGst.ts`):** the HSN code is a property of **each line item** (`CapexLineItem.hsnCode`), **not** the vendor's quote — the same item carries one HSN across all vendors. **Only the vendor enters it** (required per line in their bid table — supplier `SupplierQuoteTable`/`SupplierQuoteCards` entry variant — atomically persisted on submit via `proposeRfqQuote(..., itemHsn?)`, which patches request line items in the same state pass). **Sourcing sees the HSN / GST column read-only** in the RFQ comparison grid (no dropdown); they cannot set or override it. GST is computed **per line** = `unit × qty × gstRateForHsn(item.hsnCode)` (see `rfqLineGstAmount`/`rfqLineGstRate`/`rfqLineBreakdown`); footer charges (freight/packing/service) are **not taxed**. `rfqGstAmount(quote, items?)` sums the per-line GST (legacy fallback to the deprecated quote-level `RfqQuote.hsnCode` when no items are passed); `rfqTotal(quote, items?)` = `rfqTaxableValue` (subtotal + freight + packing + service) + item-wise GST. `lowestRfqTotal(invites, items?)` likewise takes the line items. Per-line UI shows HSN, GST rate, pre-GST subtotal, GST amount, and GST-inclusive line total. The cross-tab `storage` listener re-syncs **both** `requests` and `invites` so HSN entered in the supplier tab is visible to sourcing for GST calculation. The grid's Grand Total row shows an `incl. ₹X GST` subtitle per vendor; the supplier Grand Total shows the item-wise GST line. (`RfqQuote.hsnCode` is retained only as a legacy fallback.)
